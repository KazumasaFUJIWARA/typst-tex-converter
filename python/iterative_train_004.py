...
train_004.jsonlを使用して継続学習を行い、各エポック後にテストを実行
出力ファイル名: 004-<epocnum 0埋め2桁>.typ
"""
...
def main():
    ...
    # 初期チェックポイントの確認（003-10から開始）
    initial_checkpoint = "outputs/llama32-3b-typst-qlora-003-10"
    if not os.path.exists(initial_checkpoint):
        print(f"エラー: 初期チェックポイント {initial_checkpoint} が見つかりません。")
        print("先に003の学習を完了してください。")
        return
    ...
    for i in range(args.start_from, args.start_from + args.iterations):
        ...
        if i == args.start_from:
            ...
            train_cmd = f"""python3 python/train_llama32_3b_qlora_fixed.py \
                ...
                --lora_alpha 16 \
                --lora_dropout 0.1"""
            print(f"継続学習（003-10から）: {train_file} (from {current_checkpoint})")
        else:
            # 2回目以降は前回のエポックから継続学習
            train_cmd = f"""python3 python/train_llama32_3b_qlora_fixed.py \
                --data {train_file} \
        ...
        test_cmd = f"""python3 python/inference_llama32_3b.py \
            ...
            --peft_model_path outputs/llama32-3b-typst-qlora-004-{epoch_num} \
            --no-chunking"""
        
        test_log = f"logs/004-{epoch_num}_test_{timestamp}.log"
        print(f"テスト実行: trained/004-{epoch_num}.typ")
        ...
        if train_result == 0:
            new_checkpoint_dir = f"outputs/llama32-3b-typst-qlora-004-{epoch_num}"
