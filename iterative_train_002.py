...
train_002.jsonlを使用して継続学習を行い、各エポック後にテストを実行
出力ファイル名: 002-<epocnum 0埋め2桁>.typ
"""
...
def main():
    parser = argparse.ArgumentParser(description="継続学習・テスト・ログ保存スクリプト（10回繰り返し）")
    ...
    # 学習データファイルの確認
    train_file = "jsonl/train_002.jsonl"
    ...
    for i in range(args.start_from, args.start_from + args.iterations):
        ...
        if i == args.start_from:
            ...
            train_cmd = f"""python3 python/train_llama32_3b_qlora_fixed.py \
                ...
                --lora_alpha 16 \
                --lora_dropout 0.1"""
            print(f"継続学習（001から）: {train_file} (from {current_checkpoint})")
        else:
            # 2回目以降は前回のエポックから継続学習
            train_cmd = f"""python3 python/train_llama32_3b_qlora_fixed.py \
                --data {train_file} \
        ...
        train_log = f"logs/002-{epoch_num}_training_{timestamp}.log"
        ...
        if train_result == 0:
            new_checkpoint_dir = f"outputs/llama32-3b-typst-qlora-002-{epoch_num}"
